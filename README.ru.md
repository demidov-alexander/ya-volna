...
## Что не должно попадать в плейлист

Залайканные детские подкасты, аудиокнига, жанр, который не хочется слышать фоном, —
всё это отсекается до отбора: и в залайканной библиотеке, и в рекомендациях.

```yaml
exclusions:
  allowed_content_types: ["music"]   # по умолчанию: без подкастов и аудиокниг
  blocked_genres: ["forchildren"]    # жанровые коды провайдера
  blocked_clusters: ["dance"]        # собственные стилевые кластеры YaVolna
```

Это три разных вопроса:

- **тип контента** — чем трек является. Яндекс Музыка помечает эпизоды подкастов и главы
  аудиокниг как немузыкальный контент, и по умолчанию они в музыкальный плейлист не
  попадают. `allowed_content_types: []` отключает проверку совсем;
- **жанр** — что о треке говорит провайдер. Коды, которые реально есть в библиотеке,
  печатает `yavolna inspect-library`;
- **кластер** — куда трек отнесла сама YaVolna; список даёт `yavolna inspect-clusters`.

`yavolna validate-config` показывает действующие исключения, а каждый запуск пишет в лог,
сколько треков отсеяло каждое правило.
